...
public class Teacher extends Person implements TeacherWatcher {
    ...
    public String introduce() {
        ...
        StringBuilder klassNumbersString = new StringBuilder("");
        if (classes.size() == 0) {
            return introduceMsg.concat(" I am a Teacher. I teach No Class.");
        }
        for (int i = 0; i < classes.size(); i++) {
            if (i != 0) {
                klassNumbersString.append(", ");
            }
            klassNumbersString.append(classes.get(i).getNumber());
        }

        Integer[] list = classes.stream().map(Klass::getNumber).toArray(Integer[]::new);

        return introduceMsg.concat(MessageFormat.format(" I am a Teacher. I teach Class {0}.", klassNumbersString));
    }
